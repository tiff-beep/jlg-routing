...
export interface Agent {
  id: string; name: string; role: "partner"|"jr_partner"|"agent"|"refer_out";
  buyerFloor: number; buyerMax: number|null; sellerFloor: number; sellerMax: number|null;
  listingEligible: boolean; listingOrder: number|null; monthlyCapISA: number;
  zones: Zone[]; zoneFlags: Zone[]; devCommunities: string[];
  buyerOnly: boolean; listingsOnly: boolean; takesRentals: boolean;
  conversionOverall: number|null; conversionListing: number|null;
  notes: string; active: boolean; onVacation: boolean; returnDate: string|null;
  provisional: boolean; referOut?: boolean; listingToggleSoon?: boolean;
}
...
// Rotation order reflects last handoff dates as of 7/1 — earliest first
